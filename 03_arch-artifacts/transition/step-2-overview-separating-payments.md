...
- **決済履歴独立**
  ...
  - 決済端末からのデータをミラーリング（Mirror）により2系統に配信する仕組みを導入

- **データ連携**
  - 人事側へのデータ連携のデータソースを、マイクロサービス側に変更する
  - PayApp → Kafka経由で人事システムへ決済イベントを配信

### スコープ外
- 社食システムのUIに表示する決済履歴データのデータソース変更
  - Step 3でのUI分離まで旧App-DBを参照し続ける

## 制約 (Constraints)
- フロントエンドバックエンド分離が現段階では難しいため、UI表示のデータソースは変更しない
  - **背景**: 旧システムはプレゼンテーションとビジネスロジックの結合度が高く、段階的分離が不可能な状態
  - **対応**: Mirrorは決済端末からのデータを2系統に配信する
    1. **PayApp（新マイクロサービス）**: Kafka経由で人事システムへイベント配信
    2. **旧App-DB**: 社食アプリのUI表示用（Step 3でUI分離が完了するまで維持）
  - この二重配信により、人事連携のモダナイゼーションとUI表示の継続を両立させる
